...
import { getPurchasesAdminApi, getPurchasesApiAdmin } from 'src/purchase/PurchaseAction'
import { TablePurchase } from './TablePurchase'
import { IPurchaseState } from 'src/types/purchase'
import { TablePurchaseAdmin } from './TablePurchaseAdmin'

export const PurchaseListAdmin = () => {
  const dispatch = useDispatch()
  const { purchases, loadingPurchase } = useSelector(state => state.PurchaseReducer as IPurchaseState)

  useEffect(() => {
    dispatch(getPurchasesApiAdmin())
    return () => {}
  }, [])
                  ...
                  <div className='table-responsive'>
                    <table className='table table-hover table-nowrap'>
                      ...
                      <tbody style={{ cursor: 'pointer' }}>
                        {(!loadingPurchase && purchases && Array.isArray(purchases) && purchases.length !== 0) && purchases?.map((purchase) =>
                          <TablePurchaseAdmin
                            key={purchase?.id}
                            id={purchase?.id}
