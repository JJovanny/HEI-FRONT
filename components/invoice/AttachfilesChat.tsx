...
import { setInvoiceDataProps, setValuePostInvoiceData, validatePostInvoice } from 'src/invoice/InvoiceActions'
import { useDispatch, useSelector } from 'src/redux/hooks'
import { IFiles, IInvoice, IInvoiceState } from 'src/types/invoice'

export const AttachfilesChat = ({ index, onlyView, fileName, size, format, path, isAdmin = false }) => {
  const dispatch = useDispatch()

  const { accessToken, attachfilesChat, invoice, adminAccessToken } = useSelector(({ UserReducer, InvoiceReducer, AdminUserReducer }) => {
    ...
    return { accessToken, attachfilesChat, invoice, adminAccessToken }
  })
  ...
  return (
    <div className={onlyView ? 'col-xl-12 col-md-12' : 'col-xl-4 col-md-6'} key={fileName}>
              ...
              <>
                <div className='ms-4 pe-2 text-end'>
                  <a
                    ...
                    onClick={async (e) => {
                      attachfilesChat[index].remove = true
                      await _setValuePostInvoiceData('attachfilesChat', attachfilesChat)
                    }}
                  >
