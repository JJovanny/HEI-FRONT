...
export const InvoiceEvidencePaymentToFinancialList = ({ index, onlyView, fileName, size, format, path, isAdmin = false }) => {
  ...
  const { evidencePaymentToFinancial }: { evidencePaymentToFinancial: IFiles[] } = invoice
              ...
              <>
                <div className='ms-4 pe-2 text-end'>
                  <a
                    ...
                    onClick={async (e) => {
                        evidencePaymentToFinancial[index].remove = true
                      await _setValuePostInvoiceData('evidencePaymentToFinancial', evidencePaymentToFinancial)
