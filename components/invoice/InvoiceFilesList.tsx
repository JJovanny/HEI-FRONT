...
export const InvoiceFilesList = ({ index, onlyView, fileName, size, format, path, isAdmin = false }) => {
  ...
  const { files }: { files: IFiles[] } = invoice
              ...
              <>
                <div className='ms-4 pe-2 text-end'>
                  <a
                    ...
                    onClick={async (e) => {
                      files[index].remove = true
                      await _setValuePostInvoiceData('files', files)
